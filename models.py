...
class MultiHeadAttention(ntorch.nn.Module):
  def __init__(self,
    ...
    self.output_linear = ntorch.nn.Linear(total_value_depth, output_depth, bias=False)
...
class PositionwiseFeedForward(ntorch.nn.Module):
  def __init__(self, d_model, filter_size, dropout = 0):
    ...
    self.c1 = ntorch.nn.Conv1d(d_model, filter_size, 1).spec('embedding', 'seqlen', 'hidden')
    self.c2 = ntorch.nn.Conv1d(filter_size, d_model, 1).spec('hidden', 'seqlen', 'embedding')
...
class EncoderLayer(ntorch.nn.Module):
  def __init__(self, hidden_size, total_key_depth, total_value_depth, filter_size, num_heads,
    ...
    self.attn = MultiHeadAttention(hidden_size, total_key_depth, total_value_depth,
...
class Transformer(_NeuralNetworkLM):
  def __init__(self, TEXT, hidden_size, num_layers, num_heads, total_key_depth, total_value_depth,
                 filter_size, max_length=32, input_dropout=0.0, layer_dropout=0.0,
                 attention_dropout=0.0, relu_dropout=0.0, use_mask=False, device = 'cuda'):
        """
        Parameters:
        ...
        self.embedding_proj = ntorch.nn.Linear(self.pretrained_emb.shape[1], hidden_size, bias=False)
        ...
        self.word_proj = ntorch.nn.Linear(hidden_size, len(self.TEXT.vocab)).spec('embedding', 'out')
  ...
  def _gen_timing_signal(self, length, channels, min_timescale=1.0, max_timescale=1e4):
    """
    ...
    return NamedTensor(torch.from_numpy(signal).type(torch.FloatTensor).to(self.device), names = ('seqlen','embedding'))
